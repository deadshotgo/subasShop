...
<aside class="widget widget-product box-shadow">
    ...
    <!-- product-item start -->
    @foreach($relModalProducts as $relatedProduct)
    <div class="product-item">
        <div class="product-img">
            <a href="{{route('/det-prod/',$relatedProduct->id)}}">
                <img src="/{{$relatedProduct->images}}" alt=""/>
            </a>
        </div>
        <div class="product-info">
            <h6 class="product-title">
                <a href="{{route('/det-prod/',$relatedProduct->id)}}">{{$relatedProduct->title}}</a>
            </h6>
            <h3 class="pro-price">{{$relatedProduct->price}} грн</h3>
        </div>
    </div>
    @endforeach
